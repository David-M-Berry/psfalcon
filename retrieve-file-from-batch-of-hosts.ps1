...
.PARAMETER HostIds
One or more host IDs for which the file retrieval will be performed. If you target a single host, use the same HostId twice to create a batch.
.PARAMETER FilePath
The file path on the target hosts to retrieve.
.PARAMETER OutputDirectory
The directory where the retrieved file will be saved.
.EXAMPLE
.\RetrieveFileFromHosts.ps1 -HostIds "xxxxxxxxxxxxxxxxxxxxxxxx", "xxxxxxxxxxxxxxxxxxxxxxxxx" -FilePath "C:\Test.txt" -OutputDirectory "C:\"
#>
...
process {
    try {
        ...
        $outputFilePath = Join-Path -Path $OutputDirectory -ChildPath "$($FilePath | Split-Path -Leaf).7z"
